...
def run_ensemble_pipeline(target_col):
    ...
    rf_cv = GridSearchCV(RandomForestClassifier(class_weight='balanced', random_state=42),
                         rf_param_grid, cv=2, scoring='accuracy', n_jobs=1, verbose=1)
    ...
    X_train_tune, X_val_tune, y_train_tune, y_val_tune = train_test_split(X_train, y_train, test_size=0.1, random_state=42)
    xgb_cv = GridSearchCV(xgb.XGBClassifier(objective='multi:softmax',
                                            ...
                                            random_state=42),
                          xgb_param_grid, cv=2, scoring='accuracy', n_jobs=1, verbose=1)

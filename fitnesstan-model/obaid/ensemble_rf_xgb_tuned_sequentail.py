...
Description:
    ...
    handling two target variables: "Primary_Cluster" and "Secondary_Cluster". It performs:
      ...
      - Hyperparameter tuning via GridSearchCV for Random Forest and XGBoost using the full parameter grid with n_jobs=1,
      - Stacking with 5-fold CV (XGBoost runs without early stopping) to generate meta-features,
...
def run_ensemble_pipeline(target_col):
    ...
    # STEP D: Hyperparameter Tuning for Base Models using the full parameter grid.
    print("[DEBUG] Tuning hyperparameters using GridSearchCV...")
    rf_param_grid = {
    ...
    rf_cv = GridSearchCV(RandomForestClassifier(class_weight='balanced', random_state=42),
                         rf_param_grid, cv=3, scoring='accuracy', n_jobs=4, verbose=1)
    ...
    xgb_cv = GridSearchCV(xgb.XGBClassifier(objective='multi:softmax',
                                            num_class=len(unique_labels),
                                            eval_metric='mlogloss',
                                            verbosity=0,
                                            use_label_encoder=False,
                                            random_state=42),
                          xgb_param_grid, cv=3, scoring='accuracy', n_jobs=4, verbose=1)
    ...
    tuned_rf = RandomForestClassifier(
        ...
        random_state=42,
        n_jobs=4   # Sequential processing
    )
    tuned_xgb = xgb.XGBClassifier(
        ...
        random_state=42,
        n_jobs=4
        # Note: n_jobs for XGBClassifier defaults to 1 if not specified.
    )
    
    # STEP F: Generate Meta-Features via Stacking (5-Fold CV)
    print("[DEBUG] Generating meta-features for stacking using 5-Fold CV...")
    ...
    for train_index, val_index in tqdm(kf.split(X_train), total=5, desc="Stacking Folds"):
